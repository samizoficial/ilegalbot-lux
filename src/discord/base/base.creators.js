...
export function setupCreators(options = {}) {
    ...
    return {
        createCommand: function (data) {
            /** @defaults */
            data.type ?? (data.type = ApplicationCommandType.ChatInput);
            data.dmPermission ?? (data.dmPermission = false);
            if (options.commands?.defaultMemberPermissions) {
                data.defaultMemberPermissions ?? (data.defaultMemberPermissions = options.commands?.defaultMemberPermissions);
            }
            /** @store */
